...
const StaggeredImageSection = () => {
  ...
  return (
    <section
      id="image-section"
      className="px-10 flex gap-6 flex-wrap justify-center py-16 bg-white overflow-y-hidden"
    >
      {images.map((image, index) => (
        <div
          key={index}
          className={`rounded-xl overflow-hidden transition duration-700 ${
            visible
        ...
        >
          <Image
            ...
            width={380}
            height={512}
            style={{ height: image.height, marginTop: image.top }}
            className={`w-[382px] h- object-cover `}
          />
        </div>
      ))}
    </section>
